...
// Pantheois - Begin
// --------------------------------------------------
extern const PAN_CHAR_T PANTHEIOS_FE_PROCESS_IDENTITY[] = "OMD-C";

PANTHEIOS_CALL(PAN_CHAR_T const*) pantheios_fe_getProcessIdentity(void*)
{ return PANTHEIOS_LITERAL_STRING("OMD-C"); }
...
Logger::Logger() : m_bEnableWriteMutex(false)
{
}
...
{
  if (!DoWeNeedToDo(lev)) return;

  if (m_bEnableWriteMutex)
  {
...
void Logger::Write2(LogLevel lev, char* buffer)
{
  try
  {
    switch (lev)
...
void Logger::SetEnableWriteMutex(bool bEnableWriteMutex)
{
  m_bEnableWriteMutex = bEnableWriteMutex;
  return;
}
