...
ATU_IB_OTI::ATU_IB_OTI(const string &p_host, unsigned int p_port, int p_clientId) :
		 m_host(p_host)
		,m_port(p_port)
		...
		,m_contractExtractor(NULL)
{
    ...
    // Sunny added - [start]
    //--------------------------------------------------
    cout << "IB OTI: SVN revision no. (IB OTI): "           << string(__SVN_REV__)       << endl;
    cout << "IB OTI: SVN revision no. (Entire workspace): " << string(__SVN_WKSPC_REV__) << endl;
    //--------------------------------------------------
    // Sunny added - [end]
    //--------------------------------------------------
}
...
void ATU_IB_OTI::setConnectString(string p_connStr)
{
	...
	if ( p_connStr.length() > 4 && p_connStr.compare(p_connStr.length() - ext.length(), ext.length(), ext) == 0 ) {
		...
		m_host = Utility::resolve2IP( config->getValue("OTI.host") );

		m_port = atoi(config->getValue("OTI.port").c_str());
		m_clientId = atoi(config->getValue("OTI.client_id").c_str());
		m_account = IBString(config->getValue("OTI.account"));
		m_filenameOrderMapping = config->getValue("OTI.order_mapping_file_path");
		if (config->getValue("OTI.master").compare("true") == 0) {
			m_isMaster = true;
			cout << "****** Master account ******" << endl;
		}

		string filteredErrorCodesStr = config->getValue("OTI.filterable_error_codes");
		if (filteredErrorCodesStr.length() > 0) {
			vector<string> strCodeList;
			Toolbox::split(strCodeList, filteredErrorCodesStr, ";");
			for (vector<string>::iterator iter = strCodeList.begin(); iter != strCodeList.end(); iter++) {
				filteredErrorCodes.insert( atoi(iter->c_str()) );
				cout << "Filter: " << *iter << endl;
			}
		}

		readContracts(config->getValue("OTI.contract_file"));

	} else {
		// Currently not support pure text connect string, as order mapping file path is needed
//		vector<string> segments;
//		Toolbox::split(segments, p_connStr, "@");
//		if (segments.size() >= 4) {
//			m_host = segments[0];
//			m_port = atoi(segments[1].c_str());
//			m_clientId = atoi(segments[2].c_str());
//			m_account = IBString(segments[3]);
//			m_filenameOrderMapping = m_account + "_" + segments[2] + "_orderMapping.dat";
//
//			// if @master is attached
//			if (segments.size() > 4 && segments[4] == "master") {
//				m_isMaster = true;
//				cout << "****** Master account ******" << endl;
//			}
//		} else {
...
void ATU_IB_OTI::detach()
{
	if (m_host.size() == 0 || m_port == -1 || m_clientId == -1) {
		addLog(__LOGSOURCE__, ATU_logfeed_struct::ALERT, "%s", "s", "--->>> IP, port, and client ID are all necessary");
		return;
	}
	...
	addLog(__LOGSOURCE__, ATU_logfeed_struct::INFO, "%s", "s", "--->>> Initiating OTI connection to TWS...");
	bool bConn = m_clientSocket.eConnect(m_host.c_str(), m_port, m_clientId);
	if (bConn) {
		addLog(__LOGSOURCE__, ATU_logfeed_struct::INFO, "%s", "s", "--->>> OTI connected to TWS!");
		...
		addLog(__LOGSOURCE__, ATU_logfeed_struct::INFO, "%s", "s", "--->>> OTI is ready.");
	} else {
		addLog(__LOGSOURCE__, ATU_logfeed_struct::ALERT, "%s", "s", "--->>> OTI failed to connect to TWS.");
	}
}
...
bool ATU_IB_OTI::on_process_signalfeed(ATU_OTI_signalfeed_struct &s)
{
	...
	// The content of feedcode follows the pattern: {symbol}-{localSymbol}-{secType} or {your feedcode}_{contractId}
		...
		if (wrongFeedcode) {
			addLog(__LOGSOURCE__, ATU_logfeed_struct::WARNING, "%s", "s", "Feedcode is incorrect. it should include symbol-localSymbol-secType, or feedcodeName_contractId.");
			return false;
		}
...
//		order.allOrNone = 1;
	...
	if (s.m_order_action != "delete") {
		...
		m_orderId2feedcode[order.orderId] = s.m_feedcode;
		...
		addLog(__LOGSOURCE__, ATU_logfeed_struct::INFO, "Appended order mapping: %s", "s", line.c_str());
	}
...
bool ATU_IB_OTI::on_process_portfolio_get_working_orders(ATU_OTI_portfolio_get_working_orders_struct &p_s)
{
	...
	for (map<OrderId, int>::iterator iter = m_orderId2filledQty.begin(); iter != m_orderId2filledQty.end(); iter++) {
		...
		of.m_timestamp = Toolbox::timeStamp_full();
		of.m_market = contract.exchange;
		of.m_feedcode = m_orderId2feedcode[orderId];
...
{
	// Get today
	string strToday = Toolbox::timeStamp().substr(0, 8);
...
//	cout << ">>>>>>>>>>>>> ClientID:" << m_orderId2Order[p_orderId].clientId << ", OrderID:" << m_orderId2Order[p_orderId].orderId << endl;
	...
	addLog(__LOGSOURCE__, ATU_logfeed_struct::INFO, "Order status: orderid:%d, status:%s", "ds", p_orderId, p_status.c_str());
	...
	of.m_timestamp = Toolbox::timeStamp_full();
	of.m_market = contract.exchange;
	of.m_feedcode = !m_isMaster ? m_orderId2feedcode[p_orderId] : (contract.symbol + "-" + contract.localSymbol + "-" + contract.secType);
...
void ATU_IB_OTI::error(const int p_id, const int p_errorCode, const IBString p_errorString)
{
	...
	*/
	...
	// Notify errorfeed
	ATU_ErrorMsgStruct ef;
	ef.m_timestamp = Toolbox::timeStamp_full();

	ef.m_errormsg = boost::lexical_cast<string>(p_errorCode) + " | " + strErrorString;
	ef.m_source = "OrderID=" + m_IBOrderId2userOrderId[p_id];

	if (filteredErrorCodes.find(p_errorCode) == filteredErrorCodes.end()) {
		notify_errorfeed(&ef);
	} else {
		m_orderId2errMsg.erase(p_id);
		string filteredMsg = "Filter errorfeed: source=" + ef.m_source + "; errormsg=" + ef.m_errormsg;
		addLog(__LOGSOURCE__, ATU_logfeed_struct::INFO, "%s", "s", filteredMsg.c_str());
	}
}
...
//	cout << "------------- >>>>>>>>>>>>> ExecDetais: orderid:" << p_execution.orderId << " -> " << p_contract.symbol << endl;
	...
	if ( productCode == "" ) {
		tf->m_feedcode = !m_isMaster ? m_orderId2feedcode[p_execution.orderId] : (contract.symbol + "-" + contract.localSymbol + "-" + contract.secType);
	} else {
		tf->m_feedcode = productCode;
	}
...
void ATU_IB_OTI::addLog(string logSource, int logSeverity, char *argsfmt, char *argtype, ...) {
	va_list listPointer;
...
//void ATU_IB_OTI::readFilteredErrorCodes() {
//	// Assume that the config file accompanies with libIB_OTI.so in the same directory
//	FILE *file = fopen("ib_filtered_error_codes.ini", "r");
//	char line[101];
//	if (file != NULL) {
//		addLog(__LOGSOURCE__, ATU_logfeed_struct::INFO, "%s", "s", "ib_filtered_error_codes.ini LOADED!");
//		while (fgets(line, 101, file) && !feof(file)) {
//			// get rid of the "\n" at the end
...
//			if (strlen(line) > 0) {
//				int code = atoi(line);
////				cout << "Filter: " << code << endl;
//				filteredErrorCodes.insert(code);
//			}
//		}
//		fclose(file);
//		file = NULL;
//	} else {
//		addLog(__LOGSOURCE__, ATU_logfeed_struct::INFO, "%s", "s", "No ib_filtered_error_codes.ini!");
//	}
//}
