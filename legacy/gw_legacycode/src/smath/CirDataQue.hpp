#ifndef CIRDATAQUE_HPP
#define CIRDATAQUE_HPP
...
class CirDataQue : public DataQue<T>
{
	protected:
		...
		int _iCirDQPsn;
		int _iOpulentSize; //for speedy GetArrayInInsOrd(), fewer wrap-arounds, less memcpy
		void AddChild(T);
		void AddChild(T,T){}
		void AddChild(T,T,T){}
...
template <typename T>
bool CirDataQue<T>::WarmedUp() const
{
...
const T* CirDataQue<T>::GetArrayInInsOrd()
{
	...
	if (_vCirDQ.size() < this->_iTargetSize)
	{
		SException se;
		se.PrintMsg(__FILE__, __FUNCTION__, __LINE__);
		throw se;
	...
	//normal cases: opulent vector used
			...
			_vCirDQ.size() < _iOpulentSize &&
			_vCirDQ.size() >= this->_iTargetSize)
	{
		return &(_vCirDQ[_vCirDQ.size() - this->_iTargetSize]);
	}
	else if (_iOpulentSize > this->_iTargetSize &&
			_vCirDQ.size() == _iOpulentSize &&
			_iCirDQPsn - this->_iTargetSize >= 0)
	{
