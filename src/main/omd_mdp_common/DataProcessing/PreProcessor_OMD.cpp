...
void PreProcessor_OMD::Run()
{
  for (;;)
  {
    ...
    if (oph->PktSize > 0 && oph->PktSize <= MAX_OMD_PACKET_SIZE)
    {
      ...
      //--------------------------------------------------
      m_Logger->Write(Logger::DEBUG,"%s: %s : %u : m_RawPktCirBuf.Size()          %u", __FILENAME__, m_McastIdentifier.IP().c_str(), m_McastIdentifier.Port(), m_RawPktCirBuf->Size());
    ...
    if (iMsgCnt == 0)
    {
      m_Logger->Write(Logger::DEBUG,"%s: %s : %u : This is a HEARTBEAT packet", __FILENAME__, m_McastIdentifier.IP().c_str(), m_McastIdentifier.Port());
      m_RawPktCirBuf->PopFront();
    ...
    for (int i = 0; i < iMsgCnt; ++i)
    {
      ...
      m_Logger->Write(Logger::DEBUG,"%s: ChannelID:%u. %s : %u : Msg Seq No (unadj): %u", __FILENAME__, m_ChannelID, m_McastIdentifier.IP().c_str(), m_McastIdentifier.Port(), iUnadjSeqNo);

      unsigned short usMsgSize = READ_UINT16(pbMsg);
      unsigned short usMsgType = READ_UINT16(pbMsg+2);

      m_Logger->Write(Logger::DEBUG,"%s: MsgType: %u", __FILENAME__, usMsgType);
      m_Logger->Write(Logger::DEBUG,"%s: MsgSize: %u", __FILENAME__, usMsgSize);
        ...
        //(iUnadjSeqNo < 20 && iUnadjSeqNo < m_LastUnadjSeqNo))     // implicit seq no reset, this will cause other problems, so disabled
      {
        ...
        if (m_McastIdentifier.McastType() == McastIdentifier::REALTIME)
        {
          ...
          m_Logger->Write(Logger::NOTICE,"%s: ChannelID:%u. Refresh activated after receiving OMD_SEQUENCE_RESET.", __FILENAME__, m_ChannelID);
          // else
          //   m_Logger->Write(Logger::NOTICE,"%s: ChannelID:%u. Refresh activated after detecting sequence number reset..", __FILENAME__, m_ChannelID);
          ...
          for (its = pset->begin(); its != pset->end(); ++its)
          {
            ...
            m_Logger->Write(Logger::INFO,"%s: ChannelID:%u. Resetting OrderBook %u upon reception of OMD_SEQUENCE_RESET.", __FILENAME__, m_ChannelID, *its);
          }
        } // if (REALTIME)
        else
        {
      ...
      else
      {
        ...
        if (usMsgType == OMDC_AGGREGATE_ORDER_BOOK_UPDATE || usMsgType == OMDD_AGGREGATE_ORDER_BOOK_UPDATE)
        {
          ...
          m_Logger->Write(Logger::DEBUG,"%s: ChannelID:%u. Discovery: OrderBookID [%u] is received through this channel.", __FILENAME__, m_ChannelID, oaobu->SecurityCode);
