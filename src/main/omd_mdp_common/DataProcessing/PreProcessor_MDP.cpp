...
void PreProcessor_MDP::Run()
{
  for (;;)
  {
    ...
    //--------------------------------------------------
    m_Logger->Write(Logger::DEBUG,"PreProcessor: %s : %u ChannelID:%u. %s(%c): m_RawPktCirBuf.Size()          %u", m_McastIdentifier.IP().c_str(), m_McastIdentifier.Port(), m_ChannelID, (m_McastIdentifier.McastType() == McastIdentifier::REALTIME ? "RT" : "RF"), (m_McastIdentifier.Channel() == McastIdentifier::A ? 'A':'B'), m_RawPktCirBuf->Size());
    m_Logger->Write(Logger::DEBUG,"PreProcessor: %s : %u ChannelID:%u. %s(%c): m_RawPktCirBuf.AllocatedSize() %u", m_McastIdentifier.IP().c_str(), m_McastIdentifier.Port(), m_ChannelID, (m_McastIdentifier.McastType() == McastIdentifier::REALTIME ? "RT" : "RF"), (m_McastIdentifier.Channel() == McastIdentifier::A ? 'A':'B'), m_RawPktCirBuf->AllocatedSize());
    m_Logger->Write(Logger::DEBUG,"PreProcessor: %s : %u ChannelID:%u. %s(%c): m_RawPktCirBuf.GetPktSize()    %u", m_McastIdentifier.IP().c_str(), m_McastIdentifier.Port(), m_ChannelID, (m_McastIdentifier.McastType() == McastIdentifier::REALTIME ? "RT" : "RF"), (m_McastIdentifier.Channel() == McastIdentifier::A ? 'A':'B'), m_RawPktCirBuf->GetPktSize());

    //--------------------------------------------------
    // Output Packet Header info
    //--------------------------------------------------
    MDP_Packet_Header * mph  = (MDP_Packet_Header*)(pbPkt);
    m_Logger->Write(m_PrintPreProcSeqNoAsInfo ? Logger::INFO : Logger::DEBUG, "PreProcessor: %s : %u ChannelID:%u. %s(%c): Packet Header: PktSeqNum:  %u",
                    ...
                    mph->PktSeqNum);
    m_Logger->Write(Logger::DEBUG,"PreProcessor: %s : %u ChannelID:%u. %s(%c): Packet Header: Send Time:  %s",
                    m_McastIdentifier.IP().c_str(),
                    m_McastIdentifier.Port(),
                    m_ChannelID,
                    (m_McastIdentifier.McastType() == McastIdentifier::REALTIME ? "RT" : "RF"),
