...
{ if (!dqPeriod_Guess.empty()) return dqPeriod_Guess.back(); else return 0;}

double SgnlPrc::Period_HD_DD()
{ if (!dqPeriod_HD_DD.empty()) return dqPeriod_HD_DD.back(); else return dqPeriod_Guess.back(); }

double SgnlPrc::SNR()
{ if (!dqSNR.empty()) return dqSNR.back(); else return 0;}

double SgnlPrc::SineWaveI()
{ if (!dqSineWaveI.empty()) return dqSineWaveI.back(); else return 0;}

double SgnlPrc::SineWaveILead()
{ if (!dqSineWaveILead.empty()) return dqSineWaveILead.back(); else return 0;}

double SgnlPrc::PASMA()
{ if (!dqPASMA.empty()) return dqPASMA.back(); else return 0;}

double SgnlPrc::OBLMA()
{ if (!dq_1bl_wma_42.empty()) return dq_1bl_wma_42.back(); else return 0;}
...
void SgnlPrc::AddChild(double dH, double dL, double dC)
{
  ...
  if (!ema2_M.Ready() || !emaPA_M.Ready()) return;
...
void SgnlPrc::CalSNR()
{
  ...
  double dSNR = MAGIC_NUM;
  if (!emaNoise.Ready() || dqSignal.empty()) return;
...
void SgnlPrc::CalPASMA()
{
  ...
  if (dqHistM.size() < (unsigned int) iPeriod+1) return;
