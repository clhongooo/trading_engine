...
JSONNode genNode(OMD_Retransmission_Header &s, string name, char *buffer, int bufferLength) {
  ...
  jsn.push_back(JSONNode("SendTime",Util::fromUnixTimeToString(s.SendTime, Util::NANOSEC, Util::GMT, Util::HKT)));
return jsn;}
JSONNode genNode(OMD_Logon &s, string name, char *buffer, int bufferLength) {
  ...
  jsn.push_back(JSONNode("Username", Util::TrimR(sUsername)));
return jsn;}
...
JSONNode genNode(OMDC_Market_Definition &s, string name, char *buffer, int bufferLength) {
  ...
  jsn.push_back(JSONNode("MarketCode", Util::TrimR(sMarketCode)));
  string sMarketName(s.MarketName, sizeof(s.MarketName));
  jsn.push_back(JSONNode("MarketName", Util::TrimR(sMarketName)));
  string sCurrencyCode(s.CurrencyCode, sizeof(s.CurrencyCode));
  jsn.push_back(JSONNode("CurrencyCode", Util::TrimR(sCurrencyCode)));
...
JSONNode genNode(OMDC_Security_Definition &s, string name, char *buffer, int bufferLength) {
  ...
  jsn.push_back(JSONNode("ISINCode", Util::TrimR(sISINCode)));
  string sInstrumentType(s.InstrumentType, sizeof(s.InstrumentType));
  jsn.push_back(JSONNode("InstrumentType", Util::TrimR(sInstrumentType)));
  string sSpreadTableCode(s.SpreadTableCode, sizeof(s.SpreadTableCode));
  jsn.push_back(JSONNode("SpreadTableCode", Util::TrimR(sSpreadTableCode)));
  string sSecurityShortName(s.SecurityShortName, sizeof(s.SecurityShortName));
  jsn.push_back(JSONNode("SecurityShortName", Util::TrimR(sSecurityShortName)));
  string sCurrencyCode(s.CurrencyCode, sizeof(s.CurrencyCode));
  jsn.push_back(JSONNode("CurrencyCode", Util::TrimR(sCurrencyCode)));
  ...
  jsn.push_back(JSONNode("FreeText", Util::TrimR(sFreeText)));
  if (s.EFNFlag != 0)
...
JSONNode genNode(OMDC_Trading_Session_Status &s, string name, char *buffer, int bufferLength) {
  ...
  jsn.push_back(JSONNode("StartDateTime",Util::fromUnixTimeToString(s.StartDateTime,Util::NANOSEC,Util::GMT,Util::HKT)));
  jsn.push_back(JSONNode("EndDateTime",Util::fromUnixTimeToString(s.EndDateTime,Util::NANOSEC,Util::GMT,Util::HKT)));
return jsn;}
...
JSONNode genNode(OMDC_Trade &s, string name, char *buffer, int bufferLength) {
  ...
  jsn.push_back(JSONNode("TradeTime",Util::fromUnixTimeToString(s.TradeTime, Util::NANOSEC, Util::GMT, Util::HKT)));
return jsn;}
...
JSONNode genNode(OMDC_News &s, string name, char *buffer, int bufferLength) {
  ...
  jsn.push_back(JSONNode("NewsType", Util::TrimR(sNewsType)));
  string sNewsID(s.NewsID, sizeof(s.NewsID));
  jsn.push_back(JSONNode("NewsID", Util::TrimR(sNewsID)));
  ...
  jsn.push_back(JSONNode("ReleaseTime",Util::fromUnixTimeToString(s.ReleaseTime, Util::NANOSEC, Util::GMT, Util::HKT)));
...
JSONNode genNode(OMDC_Index_Definition &s, string name, char *buffer, int bufferLength) {
  ...
  jsn.push_back(JSONNode("IndexCode", Util::TrimR(sIndexCode)));
  if (s.IndexSource != 0)
  {
    string sIndexSource(&s.IndexSource,sizeof(s.IndexSource));
    jsn.push_back(JSONNode("IndexSource",Util::TrimR(sIndexSource)));
  }
  else
  {
...
JSONNode genNode(OMDC_Index_Data &s, string name, char *buffer, int bufferLength) {
  ...
  if (s.IndexStatus != 0)
  {
    ...
    jsn.push_back(JSONNode("IndexStatus",Util::TrimR(sIndexStatus)));
  }
  else
  {
    jsn.push_back(JSONNode("IndexStatus",""));
  }
  jsn.push_back(JSONNode("IndexTime",Util::fromUnixTimeToString(s.IndexTime, Util::NANOSEC, Util::GMT, Util::HKT)));
  ...
  if (s.Exception != 0)
  {
    ...
    jsn.push_back(JSONNode("Exception",Util::TrimR(sException)));
  }
  else
  {
...
JSONNode genNode (OMDD_Commodity_Definition &s, string name, char *buffer, int bufferLength) {
  ...
  jsn.push_back(JSONNode("BaseCurrency",Util::TrimR(sBaseCurrency)));
  ...
  jsn.push_back(JSONNode("CommodityName",Util::TrimR(sCommodityName)));
  jsn.push_back(JSONNode("NominalValue",s.NominalValue));
  string sUnderlyingCode(s.UnderlyingCode,sizeof(s.UnderlyingCode));
  jsn.push_back(JSONNode("UnderlyingCode",Util::TrimR(sUnderlyingCode)));
...
JSONNode genNode (OMDD_Class_Definition &s, string name, char *buffer, int bufferLength) {
  ...
  jsn.push_back(JSONNode("InstrumentClassID",Util::TrimR(sInstrumentClassID)));
  string sInstrumentClassName(s.InstrumentClassName,sizeof(s.InstrumentClassName));
  jsn.push_back(JSONNode("InstrumentClassName",Util::TrimR(sInstrumentClassName)));
  if (s.IsFractions != 0)
  ...
  jsn.push_back(JSONNode("SettlementCurrencyID",Util::TrimR(sSettlementCurrencyID)));
  jsn.push_back(JSONNode("EffectiveTomorrow",s.EffectiveTomorrow));
return jsn;}
JSONNode genNode (OMDD_Series_Definition_Base &s, string name, char *buffer, int bufferLength) {
  ...
  jsn.push_back(JSONNode("Symbol",Util::TrimR(sSymbol)));
  ...
  jsn.push_back(JSONNode("ExpirationDate",Util::TrimR(sExpirationDate)));
...
JSONNode genNode (OMDD_Series_Definition_Extended &s, string name, char *buffer, int bufferLength) {
  ...
  jsn.push_back(JSONNode("Modifier",s.Modifier));
  jsn.push_back(JSONNode("CommodityCode",s.CommodityCode));
  jsn.push_back(JSONNode("ExpirationDate",Util::fromGeniumDateToString(s.ExpirationDate)));
  jsn.push_back(JSONNode("StrikePrice",s.StrikePrice));
  jsn.push_back(JSONNode("ContractSize",s.ContractSize));
  string sISINCode(s.ISINCode,sizeof(s.ISINCode));
  jsn.push_back(JSONNode("ISINCode",Util::TrimR(sISINCode)));
  ...
  jsn.push_back(JSONNode("EffectiveExpDate",Util::TrimR(sEffectiveExpDate)));
  jsn.push_back(JSONNode("DateTimeLastTrading",Util::fromUnixTimeToString(s.DateTimeLastTrading,Util::NANOSEC,Util::GMT,Util::HKT)));
return jsn;}
JSONNode genNode (OMDD_Combination_Definition &s, string name, char *buffer, int bufferLength) {
  ...
  jsn.push_back(JSONNode("LegSide",Util::TrimR(sLegSide)));
...
JSONNode genNode (OMDD_Market_Status &s, string name, char *buffer, int bufferLength) {
  ...
  jsn.push_back(JSONNode("ActualStartDate",Util::TrimR(sActualStartDate)));
  string sActualStartTime(s.ActualStartTime,sizeof(s.ActualStartTime));
  jsn.push_back(JSONNode("ActualStartTime",Util::TrimR(sActualStartTime)));
  string sPlannedStartDate(s.PlannedStartDate ,sizeof(s.PlannedStartDate));
  jsn.push_back(JSONNode("PlannedStartDate",Util::TrimR(sPlannedStartDate)));
  string sPlannedStartTime(s.PlannedStartTime ,sizeof(s.PlannedStartTime));
  jsn.push_back(JSONNode("PlannedStartTime",Util::TrimR(sPlannedStartTime)));
...
JSONNode genNode (OMDD_Series_Status &s, string name, char *buffer, int bufferLength) {
  ...
  jsn.push_back(JSONNode("Suspended",Util::TrimR(sSuspended)));
return jsn;}
...
JSONNode genNode (OMDD_Estimated_Average_Settlement_Price &s, string name, char *buffer, int bufferLength) {
  ...
  jsn.push_back(JSONNode("EASType",Util::TrimR(sEASType)));
  string sInstrumentCode(s.InstrumentCode,sizeof(s.InstrumentCode));
  jsn.push_back(JSONNode("InstrumentCode",Util::TrimR(sInstrumentCode)));
