...
void PreProcessor::Run()
{
  for (;;)
  {
    ...
    if (m_Logger->DoWeNeedToDo(Logger::INFO) && oph->PktSize > 0 && oph->PktSize <= MAX_OMD_PACKET_SIZE)
    {
      ...
      m_Logger->Write(Logger::DEBUG,"PreProcessor: ChannelID:%u. %s (%c): Packet Header: Send Time:     %s", m_ChannelID, (m_McastIdentifier.McastType() == McastIdentifier::REALTIME ? "RT" : "RF"), (m_McastIdentifier.Channel() == McastIdentifier::A ? 'A':'B'), Util::fromUnixTimeToString(oph->SendTime, Util::NANOSEC, Util::GMT, Util::HKT, true).c_str());
    ...
    else if (oph->PktSize > MAX_OMD_PACKET_SIZE)
    {
      m_Logger->Write(Logger::WARNING,"PreProcessor: ChannelID:%u. %s (%c): Received erroneous packet size: Send Time: %s, PktSize = %u.",
          m_ChannelID, (m_McastIdentifier.McastType() == McastIdentifier::REALTIME ? "RT" : "RF"), (m_McastIdentifier.Channel() == McastIdentifier::A ? 'A':'B'), Util::fromUnixTimeToString(oph->SendTime, Util::NANOSEC, Util::GMT, Util::HKT, true).c_str(), oph->PktSize);
    ...
    //       m_ChannelID, (m_McastIdentifier.McastType() == McastIdentifier::REALTIME ? "RT" : "RF"), (m_McastIdentifier.Channel() == McastIdentifier::A ? 'A':'B'), Util::fromUnixTimeToString(oph->SendTime, Util::NANOSEC, Util::GMT, Util::HKT, true).c_str());
    //   m_RawPktCirBuf->PopFront();
    //   continue;
    // }
    // else
