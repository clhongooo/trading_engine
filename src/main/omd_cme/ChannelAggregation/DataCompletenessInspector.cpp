...
void DataCompletenessInspector::Run()
{
  ...
  for (;;)
  {
    ...
    if
      (!m_MsgCirBuf->GetLatestSeqNo(uiLatestAdjSeqNo)               ||
       !m_MsgCirBuf->GetSmallestMissingSeqNo(uiSmlMissingAdjSeqNo)  ||
       !m_MsgCirBuf->GetLargestMissingSeqNo(uiLrgtMissingAdjSeqNo))
      {
    ...
    else
    {
      bool bDirFlag = false;
      uint64_t uiTStamp = 0;
      if (m_MsgCirBuf->CheckDirtyFlagTStamp(uiSmlMissingAdjSeqNo,bDirFlag,uiTStamp))
      {
        if (!bDirFlag)
        {
          uint64_t uiCurTime = Util::getCurrentUnixTime();
          ...
          if (uiLatestAdjSeqNo > uiSmlMissingAdjSeqNo + m_TriggerRefreshSeqNoGap)
          {
            ...
            if (!m_ShrObj->CheckRefreshActivatnStatus(m_ChannelID))
            {
              ...
              // must print log
              m_Logger->Write(Logger::NOTICE, "DataCompletenessInspector: ChannelID:%u. Latest Seq No (adj): %u, Smallest Missing Seq No (adj): %u (TStamp=%s), Largest Missing Seq No (adj): %u",
                  m_ChannelID,
                  uiLatestAdjSeqNo, uiSmlMissingAdjSeqNo,
                  Util::fromUnixTimeToString(uiTStamp, Util::NANOSEC, Util::HKT, Util::HKT).c_str(),
          ...
          else if (uiLatestAdjSeqNo > uiSmlMissingAdjSeqNo + m_TriggerRetransmissionSeqNoGap)
          {
            ...
            if (bCanPrintRTSLog)
              m_Logger->Write(Logger::NOTICE, "DataCompletenessInspector: ChannelID:%u. Latest Seq No (adj): %u, Smallest Missing Seq No (adj): %u (TStamp=%s), Largest Missing Seq No (adj): %u",
                  m_ChannelID, uiLatestAdjSeqNo, uiSmlMissingAdjSeqNo,
                  Util::fromUnixTimeToString(uiTStamp, Util::MILLISEC, Util::HKT, Util::HKT).c_str(),
                  uiLrgtMissingAdjSeqNo);
